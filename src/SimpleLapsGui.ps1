...
2025-12-08 / v1.3i / htcfreek / Main UI redisign; Multi domain support; Zoom dialog for bigger crdential preview; Set time dialog now respects system's format settings.; Support for ESCAPE key in time dialog and about dialog.; Fixed scaling bug in about dialog.
Removed AutoIT-Exe-Wrapper; Various improvements.
...
$copyIconImage = [System.Drawing.Image]::FromStream($copyIconStream)
#---
...
[string]$Script:selectedComputerIdentity = ""
[string]$Script:selectedComputerDomain = $null
...
function GetPassword
{
    ...
    $Script:selectedComputerIdentity = ""
    $Script:selectedComputerDomain = $null
    $Script:winLapsComputer = $false
    #---
    $AccountNameData.Text = " "
    $AccoutNameCopyButton.Hide()
    $AccoutNamePreviewButton.Hide()
    $PasswordData.Hide()
    ...
    if ($inputComputerName -match "^CN=.*,.*DN=.*") {
        $statusText.Text = "Failed: Distinguished name (DN) is not supported!"
        $statusText.ForeColor = "Red"
        return
    }
    ...
    if ($inputComputerName -eq ".\") {
        ...
        $Script:selectedComputer = $env:COMPUTERNAME
        $Script:selectedComputerIdentity = $env:COMPUTERNAME
        $Script:selectedComputerDomain = $null
    }
    elseif ($inputComputerName -match ".+\\.+") {
        $splitName = ($inputComputerName -split "\\")
        $Script:selectedComputer = $splitName[1]
        $Script:selectedComputerIdentity = $splitName[1]
        $Script:selectedComputerDomain = $splitName[0]
    }
    elseif ($inputComputerName -match ".+\..+") {
        $splitName = ($inputComputerName -split "\.",2)
        $Script:selectedComputer = $splitName[0]
        $Script:selectedComputerIdentity = $inputComputerName
        $Script:selectedComputerDomain = $splitName[1]
    }
    else {
        $Script:selectedComputer = $inputComputerName
        $Script:selectedComputerIdentity = $inputComputerName
        $Script:selectedComputerDomain = $null
    }
    ...
    try
    {
        if ([string]::IsNullOrWhiteSpace($Script:selectedComputerDomain)) {
           $pwds = Get-LapsADPassword -Identity "$($Script:selectedComputerIdentity)" -AsPlainText -IncludeHistory -ErrorAction Stop -WarningAction SilentlyContinue | Select-Object ComputerName,Account,Password,PasswordUpdateTime,ExpirationTimestamp
        }
        else {
            $pwds = Get-LapsADPassword -Identity "$($Script:selectedComputerIdentity)" -Domain "$($Script:selectedComputerDomain)" -AsPlainText -IncludeHistory -ErrorAction Stop -WarningAction SilentlyContinue | Select-Object ComputerName,Account,Password,PasswordUpdateTime,ExpirationTimestamp
        }
    }
    catch
    {
        ...
        {
            $statusText.Text = "Failed: This tool must be run on an AD domain-joined machine!"
            $statusText.ForeColor = "Red"
            return
        }
        else
    ...
    if ($Script:winLapsComputer) {
        # Legacy Laps does not support account
        $AccountNameData.Text = $pwds[0].Account
        $AccoutNameCopyButton.Show()
        $AccoutNamePreviewButton.Show()
    }
    $PasswordData.Text = $pwds[0].Password
    #$PasswordData.Text = " " + $pwds[0].Password
    $passwordToolTip = New-Object System.Windows.Forms.ToolTip
    $passwordToolTip.SetToolTip($PasswordData, $pwds[0].Password)
	$PasswordData.Show()
    $PasswordCopyButton.Show()
    $PasswordPreviewButton.Show()
    $ExpirationTimeData.Text = $pwds[0].ExpirationTimestamp.ToString()
    if (!$Script:winLapsComputer -and ![string]::IsNullOrWhiteSpace($Script:selectedComputerDomain)) {
    ...
    if ($pwds.length -gt 1)
    {
        for ($i=1; $i -lt $pwds.length; $i++) {
            ...
            $item.SubItems.Add($pwds[$i].Account)
...
function SetExpirationTime([ref]$pForm)
{
    ...
    $goButton.Enabled = $False
    $AccoutNameCopyButton.Enabled = $false;
        ...
        try {
           # Execute
            if ($script:winLapsComputer) {
                ...
                if (![string]::IsNullOrWhiteSpace($Script:selectedComputerDomain)) {
                    # custom domain
                    Set-LapsADPasswordExpirationTime -Identity $Script:selectedComputerIdentity -Domain $Script:selectedComputerDomain -WhenEffective $newExpTime -ErrorAction Stop -WarningAction SilentlyContinue
                }
                else {
                    # defautl domain
                    Set-LapsADPasswordExpirationTime -Identity $Script:selectedComputerIdentity -WhenEffective $newExpTime -ErrorAction Stop -WarningAction SilentlyContinue
                }
            }
            else {
                # LegacyLaps
...
if ($currentDTFormat.ShortTimePattern -like '*tt') {
    # if with am/pm, we nedd wider input field
    $dateTime.Width = "200"
...
$CompInfoText.Size = New-Object System.Drawing.Point(600,15)
...
$AccoutNameCopyButton = New-Object System.Windows.Forms.Button
$AccoutNameCopyButton.Location = New-Object System.Drawing.Size(400,109)
$AccoutNameCopyButton.Size = New-Object System.Drawing.Size(75,25)
$AccoutNameCopyButton.Text = "Copy"
$AccoutNameCopyButton.Font = $textFont
$AccoutNameCopyButton.Add_Click({Set-Clipboard $AccountNameData.Text})
$AccoutNameCopyButton.Hide()
$mainForm.Controls.Add($AccoutNameCopyButton)


$AccoutNamePreviewButton = New-Object System.Windows.Forms.Button
$AccoutNamePreviewButton.Location = New-Object System.Drawing.Size(480,109)
$AccoutNamePreviewButton.Size = New-Object System.Drawing.Size(25,25)
$AccoutNamePreviewButton.Text = ""
$AccoutNamePreviewButton.AccessibleName = "Zoomed preview"
$AccoutNamePreviewButton.Font = $textFont
$AccoutNamePreviewButton.Add_Click({
    $zoomDlgAccount.Text = $AccountNameData.Text;
    $zoomDlgPassword.Text = $PasswordData.Text;
    $zoomDialog.ShowDialog()})
$AccoutNamePreviewButton.Image = $magnifyIconImage 
$AccoutNamePreviewButton.Hide()
$mainForm.Controls.Add($AccoutNamePreviewButton)
...
#$PasswordData.Location = New-Object System.Drawing.Point(85,141)
$PasswordData.Size = New-Object System.Drawing.Point(310,20)
#$PasswordData.Size = New-Object System.Drawing.Point(295,20)
$PasswordData.Font = $pwdFont
$PasswordData.ForeColor = "Black"
$PasswordData.Text = ""
$PasswordData.AutoEllipsis = $true
#$PasswordData.ReadOnly = $true
#$PasswordData.BackColor = "White"
#$PasswordData.BorderStyle = "None"
#$PasswordData.ContextMenu = New-Object System.Windows.Forms.ContextMenu
...
$PasswordPreviewButton.AccessibleName = "Zoomed preview"
$PasswordPreviewButton.Font = $textFont
$PasswordPreviewButton.Add_Click({
    $zoomDlgAccount.Text = $AccountNameData.Text;
    $zoomDlgPassword.Text = $PasswordData.Text;
    $zoomDialog.ShowDialog()})
$PasswordPreviewButton.Image = $magnifyIconImage
$PasswordPreviewButton.Hide()
...
$ExpirationTimeLabel.Text = "Expires at:"
...
$TimeSetNotSupportedText.Location = New-Object System.Drawing.Point(310,164)
...
[void]$ListPwdHistory.Columns.Add('Expired at',122)
...
$menuItem11.Image = $copyIconImage
[void]$contextMenu.Items.Add("-")
$menuItem12 = $contextMenu.Items.Add("Zoomed preview")
$menuItem12.Font = $pwdHistoryHeaderFont
$menuClick12 = ({
    $zoomDlgAccount.Text = $ListPwdHistory.SelectedItems[0].SubItems[1].Text;
    $zoomDlgPassword.Text = $ListPwdHistory.SelectedItems[0].SubItems[2].Text;
    $zoomDialog.ShowDialog()})
