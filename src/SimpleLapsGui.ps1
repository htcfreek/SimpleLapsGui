...
function GetPassword
{
    ...
    catch
    {
        ...
        {
            $statusText.Text = "Failed: This program must be run on an AD domain-joined machine!"
            $statusText.ForeColor = "Red"
            return
        }
        else
    ...
    if ($pwds.length -gt 1)
    {
        for ($i=1; $i -lt $pwds.length; $i++) {
            ...
            if ($pwds[$i].Account.Length -gt "User name".Length) {
                $resizeAccountColumnStyle = [System.Windows.Forms.ColumnHeaderAutoResizeStyle]::ColumnContent
            }
            if ($pwds[$i].Password.Length -gt "Password".Length) {
    ...
    if ($ListPwdHistory.Items.Count -eq 0)
    {
        ...
        # Resize columns as needed for pwd info and nice view
    ...
    if ($dlgResult -eq [System.Windows.Forms.DialogResult]::OK)
    {
        ...
        try {
           # Execute
            ...
            # - Sleep before to give change command enough time for execution
            Sleep -Seconds 5
            if (![string]::IsNullOrWhiteSpace($Script:selectedComputerDomain)) {
                # custom domain
                $newCompTime = Get-LapsADPassword -Identity $Script:selectedComputerIdentity -Domain $Script:selectedComputerDomain -ErrorAction SilentlyContinue -WarningAction SilentlyContinue | Select-Object ExpirationTimestamp
            }
            else {
                # defautl domain
                $newCompTime = Get-LapsADPassword -Identity $Script:selectedComputerIdentity -ErrorAction SilentlyContinue -WarningAction SilentlyContinue | Select-Object ExpirationTimestamp
            }
...
function Set-MainWindowControlsDisabled {
    ...
    $AccoutNamePreviewButton.Enabled = $false
...
function Set-MainWindowControlsEnabled {
    ...
    $AccoutNamePreviewButton.Enabled = $true
